...
static int wiring_open(PROGRAMMER *pgm, const char *port) {
    ...
    if (WIRINGPDATA(pgm)->snoozetime > 0) {
        ...
        while (timetosnooze--)
            emscripten_sleep(1000/1000); // replace usleep with emscripten_slee
        ...
        if((100+delay) > 0)
            emscripten_sleep((100+delay)*1000/1000); // Wait until board comes out of reset // replace usleep with emscripten_slee
    }
