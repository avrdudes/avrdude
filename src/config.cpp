...
COMMENT *locate_comment(const LISTID comments, const char *where, int rhs) {
    if(comments)
        for(LNODEID ln=lfirst(comments); ln; ln=lnext(ln)) {
            COMMENT *n = static_cast<COMMENT *>(ldata(ln));
...
static void addcomment(int rhs) {
    if(lkw) {
        COMMENT *node = static_cast<COMMENT *>(cfg_malloc("addcomment()", sizeof(*node)));
        node->rhs = rhs;
...
static int cmp_comp(const void *v1, const void *v2) {
    const Component_t *c1 = static_cast<const Component_t *>(v1), *c2 = static_cast<const Component_t *>(v2);
...
Component_t *cfg_comp_search(const char *name, int strct) {
    ...
    key.name = name;
    key.strct = strct;
    return static_cast<Component_t *>(bsearch(&key, avr_comp, sizeof avr_comp / sizeof *avr_comp, sizeof(Component_t),
                                              cmp_comp));
}
