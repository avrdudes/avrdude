...
static int ch341a_spi_cmd(const PROGRAMMER *pgm, const unsigned char *cmd,
                          unsigned char *res);
static int ch341a_spi(const PROGRAMMER *pgm, const unsigned char *in,
...
static int ch341a_spi_transfer(const PROGRAMMER *pgm, const unsigned char *cmd,
                               unsigned char *res);
// dummy functions
...
static int CH341USBTransferPart(const PROGRAMMER *pgm,
                                enum libusb_endpoint_direction dir,
                                unsigned char *buff, unsigned int size)
{
  int ret, bytestransferred;

  if (!PDATA(pgm)->usbhandle) return 0;

  if ((ret = libusb_bulk_transfer(PDATA(pgm)->usbhandle,
                                  CH341A_USB_BULK_ENDPOINT | dir, buff, size,
                                  &bytestransferred, CH341A_USB_TIMEOUT))) {
    pmsg_error("libusb_bulk_transfer for IN_EP failed: %d (%s)\n",
...
static bool CH341USBTransfer(const PROGRAMMER *pgm,
                             enum libusb_endpoint_direction dir,
                             unsigned char *buff, unsigned int size)
{
                                                       ...
                                                       unsigned char *res)
{
  ...
  pmsg_debug("ch341a_spi_transfer(0x%02x, 0x%02x, 0x%02x, 0x%02x)%s",
      cmd[0], cmd[1], cmd[2], cmd[3], verbose > 3 ? "...\n" : "\n");

  if (size > CH341A_PACKET_LENGTH - 1) size = CH341A_PACKET_LENGTH - 1;

  for (i = 0; i < size; i++) {
    pkt[i] = cmd[i];
  }

  if (CH341USBTransferPart(pgm, LIBUSB_ENDPOINT_OUT, pkt, size) <= 0 ||
      CH341USBTransferPart(pgm, LIBUSB_ENDPOINT_IN, pkt, size) <= 0) {
    pmsg_error("failed to transfer data to/from CH341\n");
    return -1;
  }
                      ...
                      unsigned char *out, int size)
{
  ...
  for (i = 0; i < size; i++) pkt[i + 1] = swap_byte(in[i]);
  ...
  for (i = 0; i < size; i++) out[i] = swap_byte(pkt[i]);
...
static int ch341a_spi_program_enable(const PROGRAMMER *pgm, const AVRPART *p)
{
  ...
  cmd[0] = 0;
  pmsg_trace("ch341a_program_enable() %s\n", p->op[AVR_OP_PGM_ENABLE]);
  ...
  pmsg_debug("program_enable(): sending command. Resp = %x %x %x %x \n",
            (int)res[0], (int)res[1], (int)res[2], (int)res[3]);
  // check for sync character
  if (res[2] != cmd[1]) return -2;
  return 0;
}

static int ch341a_spi_chip_erase(const struct programmer_t *pgm,
                                 const AVRPART *p)
{
  unsigned char cmd[4];
  unsigned char res[4];
...
static void ch341a_setup(PROGRAMMER *pgm)
{
  if ((pgm->cookie = malloc(sizeof(struct pdata))) == 0) {
    pmsg_error("ch341a_setup(): Out of memory allocating private data\n");
    exit(1);
  }
