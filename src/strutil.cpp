...
char *str_sprintf(const char *fmt, ...) {
    ...
    char *p = static_cast<char *>(cfg_malloc(__func__, size));
...
char *str_fgets(FILE *fp, const char **errpp) {
    ...
    while(ret[bs-2] != 0 && ret[bs-2] != '\n' && ret[bs-2] != '\r') {
        ...
        int was = bs;
        bs = 2*bs+1;
        ret = static_cast<char *>(cfg_realloc(__func__, ret, bs));
...
void change_endian(void *p, int size) {
    uint8_t tmp, *w = static_cast<uint8_t *>(p);
...
Str2data *str_todata(const char *s, int type, const AVRPART *part, const char *memstr) {
    char *end_ptr;
    Str2data *sd = static_cast<Str2data *>(cfg_malloc(__func__, sizeof *sd));
    ...
    if(type & STR_STRING && arglen > 1) { // Try C-style string or single character
        if((*str == '\'' && str[arglen-1] == '\'') || (*str == '\"' && str[arglen-1] == '\"')) {
            char *s = static_cast<char *>(calloc(arglen - 1, 1));
            if (s == NULL)
    ...
    if(type & STR_FILE && part && memstr) { // File name containing data to be loaded
        ...
        int rc = fileio(FIO_READ_FOR_VERIFY, str, static_cast<FILEFMT>(format), dp, memstr, -1);
        if(rc < 0) {
            avr_free_part(dp);
            Return("unable to read the%s %s file", fmtstr, fileio_fmtstr(static_cast<FILEFMT>(format)));
        }
...
int str_levenshtein(const char *str1, const char *str2,
    ...
    int *row0 = static_cast<int *>(cfg_malloc(__func__, (len2 + 1) * sizeof *row0));
    int *row1 = static_cast<int *>(cfg_malloc(__func__, (len2 + 1) * sizeof *row1));
    int *row2 = static_cast<int *>(cfg_malloc(__func__, (len2 + 1) * sizeof *row2));
...
size_t str_weighted_damerau_levenshtein(const char *s1, const char *s2) {
    ...
    size_t *row0 = static_cast<size_t *>(cfg_malloc(__func__, (len2 + 1) * sizeof *row0));
    size_t *row1 = static_cast<size_t *>(cfg_malloc(__func__, (len2 + 1) * sizeof *row1));
    size_t *row2 = static_cast<size_t *>(cfg_malloc(__func__, (len2 + 1) * sizeof *row2));
