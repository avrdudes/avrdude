...
static int usb_control(const PROGRAMMER *pgm, unsigned int requestid,
  unsigned int val, unsigned int index, int silent) {
...
static int usb_in(const PROGRAMMER *pgm,
  unsigned int requestid, unsigned int val, unsigned int index, unsigned char *buffer, int buflen, int bitclk) {
  int nbytes;
  int timeout;
  int i;

  /*
   * Calculate the amount of time we expect the process to take by figuring the
   * bit-clock time and buffer size and adding to the standard USB timeout.
   */
...
/*
 * Encode 1 start bit (0), 8 data bits, 1 parity, 2 stop bits (1) inside 16
 * bits. The data is padded to 16 bits by 4 leading 1s (which will be ignored
 * since they're not start bits). This layout enables a write to be followed by
 * a read
 */
static unsigned short tpi_frame(unsigned char b) {
  return LITTLE_TO_BIG_16(0xf000 | (reverse(b) << 3) | tpi_parity(b) << 2 | TPI_STOP_BITS);
}

// Transmit a single byte encapsulated in a 32-bit transfer; unused bits are padded with 1s
...
// Transmit a two bytes encapsulated in a 32-bit transfer; unused bits are padded with 1s
static int usbtiny_tpi_txtx(const PROGRAMMER *pgm, unsigned char b0, unsigned char b1) {
  unsigned char res[4];
...
/*
 * Transmit a byte then receive a byte, all encapsulated in a 32-bit transfer.
 * Unused bits are padded with 1s.  This code assumes that the start bit of the
 * byte being received arrives within at most 2 TPICLKs.  We ensure this by
 * calling avr_tpi_program_enable() with delay==TPIPCR_GT_0b.
 */
static int usbtiny_tpi_txrx(const PROGRAMMER *pgm, unsigned char b0) {
  ...
  while(w < 0)
    w <<= 1;
  /*
   * Now that we found the start bit, the top 9 bits contain the start bit and
   * the 8 data bits, but the latter in reverse order
   */
  r = reverse(w >> 7);
...
/*
 * Sometimes we just need to know the SPI command for the part to perform a
 * function. Here we wrap this request for an operation so that we can just
 * specify the part and operation and it'll do the right stuff to get the
 * information from AvrDude and send to the USBtiny.
 */
static int usbtiny_avr_op(const PROGRAMMER *pgm, const AVRPART *p, int op, unsigned char *res) {
  unsigned char cmd[4];
...
int usbtiny_cmd_tpi(const PROGRAMMER *pgm, const unsigned char *cmd, int cmd_len, unsigned char *res, int res_len) {
  unsigned char b0, b1;
  int tx, rx, r;
...
static int usbtiny_paged_load(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *m,
  ...
  for(; addr < maxaddr; addr += chunk) {
    ...
    // Send the chunk of data to the USBtiny with the function we want to perform
    if(usb_in(pgm, function,    // EEPROM or flash
...
static int usbtiny_paged_write(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *m,
  ...
  for(; addr < maxaddr; addr += chunk) {
    ...
    if(usb_out(pgm, function,   // Flash or EEPROM
