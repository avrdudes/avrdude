...
int jtagmkII_send(const PROGRAMMER *pgm, unsigned char *data, size_t len) {
    ...
    if ((buf = static_cast<unsigned char *>(malloc(len + 10))) == NULL)
    {
            ...
            case sTOKEN:
                if (c == TOKEN) {
                    ...
                    if (msglen > MAX_MESSAGE) {
                        ...
                        state = sSTART;
                        headeridx = 0;
                    } else if (NULL == (buf = static_cast<unsigned char *>(malloc(msglen + 10)))) {
                        pmsg_error("out of memory\n");
                        ignorpkt++;
                    } else {
                        memcpy(buf, header, 8);
                                ...
                                unsigned int addr, unsigned int n_bytes)
{
    ...
    if ((cmd = static_cast<unsigned char *>(malloc(page_size + 10))) == NULL) {
        pmsg_error("out of memory\n");
        return -1;
    }
...
static int jtagmkII_paged_write32(const PROGRAMMER *pgm, const AVRPART *p_unused , const AVRMEM *m,
    ...
    if ((cmd = static_cast<unsigned char *>(malloc(pgm->page_size + 10))) == NULL) {
        pmsg_error("out of memory\n");
        return -1;
    }
