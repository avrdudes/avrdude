...
char *str_utoa(unsigned n, char *buf, int base) {
  ...
  cp = buf;
  // Divide by base until the number disappeared, but ensure at least one digit will be emitted
  do {
    q = n%base;
    n /= base;
    *cp++ = q < 10? q + '0': q + 'a' - 10;
...
/*
 * str_todata() is the workhorse for generic string to data conversion for the
 * terminal write function, but is also used for generic string to integer
 * conversions in str_int() below. Both routines define the "character" of how
 * avrdude understands strings in (most) of its dealings. The granularity of
 * type is an bitwise-or combination of bits making up STR_INTEGER; STR_FLOAT;
 * STR_DOUBLE or STR_STRING. The arguments part and memstr are only needed for
 * input from files.
 */
        ...
        if(nu == 0 || toupper(*end_ptr & 0xff) == 'U' || toupper(str[arglen - 1] & 0xff) == 'U') {
          ...
          if(nl == 0 && ns == 0 && nh == 0) {   // No explicit data size
            ...
            if(is_hex) {
              ...
              sd->size = sd->ull > UINT32_MAX? 8: sd->ull > UINT16_MAX? 4: sd->ull > UINT8_MAX? 2: 1;
            }
    ...
    if(sd->size) {
      ...
      if(is_signed && is_out_of_range)
        Warning("%s out of int%d range, interpreted as %d-byte %lld%sU",
          stri, sd->size*8, sd->size, (long long int) sd->ll, sd->size == 4? "L": sd->size == 2? "H": "HH");
 ...
 *
 * Str points to a string that contains an integer terminal data item. Type can
 * be STR_INTEGER or a non-zero bitwise-or combination of integer size
 * designators STR_1, STR_2, STR_4 and STR_8 and sign type STR_SIGNED or,
 * independently, STR_UNSIGNED. A corresponding range check will be done for
 * the numbers that are encoded in the string. If neither or both of
 * STR_UNSIGNED and STR_SIGNED was given then the admitted integer can be in
 * either the signed or the unsigned range of the given size, otherwise only
 * numbers of the requested signedness range will be admitted. Either way, if
 * the string itself restricts the size through a size suffix (see below) then
 * any overflow in there will trigger a range error, too. As in C, a
 * sign-changed unsigned number will yield another unsigned number greater than
 * or equal to zero. As such, the numbers 0, -255U, -254U, ..., -1U are all
 * valid unsigned representations of one-byte integers 0, 1, ... 255. At the
 * same time -256U and 256U are not in the one-byte unsigned range [0, 255].
 * Finally, in the case of  success str_int() will set the character pointer
 * pointed to by errptr to NULL and return an integer in the range of the
 * requested type as unsigned long long. In the case of a conversion error, the
 * pointer pointed to by errptr will be set to a human-readable error message
 * whilst the returned value has no meaning.
 *
 * Integer terminal data items are either a literal C-like character such as
 * '\t' or an integer string with optional leading white space; optional + or -
 * sign; a binary (leading 0b), octal (leading 0), decimal or hexadecimal
 * number (leading 0x); optional size suffix LL/L/S/H/HH; optional unsigned U
 * suffix. All terminal data items can have an optional trailing comma to allow
 * cutting and pasting lists, and will undergo automated data size and base
 * detection. The known integer sizes are either 1 (suffix HH), 2 (suffix H or
 * S), 4 (suffix L) or 8 bytes (suffix LL).
 *
 * Usage example:
 *
 * #include "libavrdude.h"
 ...
 * whilst keeping other backslashed characters. Used for terminal line parsing
 * and reading files with ASCII numbers.
 */
 ...
 *
 * This function implements the Damerau-Levenshtein algorithm to calculate a
 * distance between strings.
 *
 * Basically, it says how many letters need to be swapped, substituted, deleted
 * from, or added to str1, at least, to get str2.
 *
 * The idea is to build a distance matrix for the substrings of both strings.
 * To avoid a large space complexity, only the last three rows are kept in
 * memory (if swaps had the same or higher cost as one deletion plus one
 * insertion, only two rows would be needed).
 *
 * At any stage, "i + 1" denotes the length of the current substring of str1
 * that the distance is calculated for.
 *
 * row2 holds the current row, row1 the previous row (i.e. for the substring of
 * str1 of length "i"), and row0 the row before that.
 *
 * In other words, at the start of the big loop, row2[j + 1] contains the
 * Damerau-Levenshtein distance between the substring of str1 of length "i" and
 * the substring of str2 of length "j + 1".
 *
 * All the big loop does is determine the partial minimum-cost paths.
 *
 * It does so by calculating the costs of the path ending in characters i (in
 * str1) and j (in str2), respectively, given that the last operation is a
 * substitution, a swap, a deletion, or an insertion.
 *
    ...
    for(size_t k1 = 0; k1 < 128; k1++)
      for(size_t k2 = 0; k2 < 128; k2++)
        wmat[k1][k2] =
          ...
          isalpha(k1) && isalpha(k2) && tolower(k1) == tolower(k2)? w/8: qwertydist(w, k1, k2);
