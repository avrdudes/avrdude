...
#define fpput(x)                                                \
  ...
  if (txSequence >= 0) {
    ...
    xbeedev_stats_send(xbs, detail, detailSequence,
                       frameGroup, txSequence, static_cast<xbee_stat_is_retry>(0), &time);
  }
  ...
  if (apiType != 0x08) {
    ...
    if (apiType != 0x21 && xbs->sourceRouteChanged) {
      ...
      int rc = sendAPIRequest(xbs, 0x21, /* Create Source Route */
                              ...
                              static_cast<xbee_stat_is_retry>(0), /* Not a retry */
                        ...
                        union filedescriptor *fdp)
{
   ...
   */
  char *ttySeparator = (char *)strchr(port, '@');
  if (ttySeparator == NULL) {
    pmsg_error("XBee: bad port syntax, require <xbee-address>@<serial-device>\n");
    return -1;
  }

  struct XBeeBootSession *xbs = static_cast<XBeeBootSession *>(malloc(sizeof(struct XBeeBootSession)));
  if (xbs == NULL) {
  ...
  while (buflen > 0) {
     ...
     */
    {
      ...
      xbeedev_stats_send(xbs, "send() hints possible triggered RECEIVE",
                         nextSequence,
                         XBEE_STATS_RECEIVE,
                         nextSequence, static_cast<xbee_stat_is_retry>(0), &sendTime);
    }
