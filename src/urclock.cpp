...
static int cmpblhash(const void *va, const void *vb) {
    const Blhash_t *a = static_cast<const Blhash_t *>(va), *b = static_cast<const Blhash_t *>(vb);
...
static int ur_initstruct(const PROGRAMMER *pgm, const AVRPART *p) {
    ...
    // No urboot bootloaders on AVR32 parts, neither on tiny devices
        ...
        goto alldone;
    {
                ...
                if (ur.blend > ur.blstart) { // Read bootloader to identify jump to vbl vector
                    ...
                    for (i = 0; i < npages; i++) {
                        ...
                        if ((rc = ur_readEF(pgm, p, spc, ur.blstart + i * flm->page_size, flm->page_size, 'F')))
                            return rc;
                        for (n = flm->page_size / 2, q = spc, j = 0;
                             j < n; j++, q += 2, toend -= 2) { // Check 16-bit opcodes
                            opcode = buf2uint16(q);
        ...
        if (havemetadata && ur.pfend >= nmeta(254, flm->size)) {
            ...
            if (ur.showall || (ur.showid && *ur.iddesc && *ur.iddesc != 'E') || ur.showapp ||
                ...
                if (spc[nm - 1] != 0xff) {
                    int32_t storesize =
                            ur.uP.flashsize > (1 << 16) ? buf2uint32(spc + nm - 5) : buf2uint16(spc + nm - 3);
                    int32_t storestart =
                            ur.uP.flashsize > (1 << 16) ? buf2uint32(spc + nm - 9) : buf2uint16(spc + nm - 5);
                        ...
                        if (!mcode) {
                            ...
                            if (yyyy > 0 && yyyy < 3000 && mm > 0 && mm < 13 && dd > 0 && dd < 32 &&
                                ...
                                if (mcode > 1) {   // Copy application name over
                                    rc = ur_readEF(pgm, p, spc, ur.pfend + 1 - nmeta(mcode, ur.uP.flashsize), mcode,
                                                   'F');
                                    if (rc < 0)
                                        return rc;
        ...
        int single = !ur.showall && (!!ur.showid + !!ur.showapp + !!ur.showstore + !!ur.showmeta +
                                     !!ur.showboot + !!ur.showversion + !!ur.showvector + !!ur.showpart +
                                     !!ur.showdate +
                                     !!ur.showfilename) == 1;
    ...
    if(!emulated) {               // Write jump to boot section to reset vector
        if(ur.boothigh && ur.blstart && ur.vbllevel==1) {
            ...
            if(flm && flm->page_size >= vecsz) {
                unsigned char *page = static_cast<unsigned char *>(cfg_malloc(__func__, flm->page_size));
...
#define NA   0, NULL, 0
    ...
    for(LNODEID ln = lfirst(extparms); ln; ln = lnext(ln)) {
        const char *extended_param = static_cast<const char *>(ldata(ln));
