...
int jtag3_open_common(PROGRAMMER *pgm, const char *port, int mode_switch) {
  ...
  if (rv < 0) {
    ...
    for(LNODEID ln=lfirst(pgm->id); ln; ln=lnext(ln)) {
      if (str_starts((const char *)ldata(ln), "snap") || str_starts((const char *)ldata(ln), "pickit4")) {
        bool is_snap_pgm = str_starts((const char *)ldata(ln), "snap");
...
void jtag3_print_parms1(const PROGRAMMER *pgm, const char *p, FILE *fp) {
  ...
  for(LNODEID ln=lfirst(pgm->id); ln; ln=lnext(ln)) {
    if(str_starts((const char *)ldata(ln), "powerdebugger")) {
...
static int jtag3_paged_write_tpi(const PROGRAMMER *pgm, const AVRPART *p,
  ...
  if ((cmd = static_cast<unsigned char *>(malloc(page_size + 9))) == NULL) {
    pmsg_error("out of memory\n");
    return -1;
  }
