...
int bitbang_initialize(const PROGRAMMER *pgm, const AVRPART *p) {
    ...
    if (p->prog_modes & PM_TPI) {
        ...
        emscripten_sleep(128000/1000);	/* wait t_TOUT (32-128ms) */ // replace usleep with emscripten_slee
    ...
    emscripten_sleep(20000/1000); /* 20 ms XXX should be a per-chip parameter */ // replace usleep with emscripten_slee
