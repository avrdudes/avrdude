...
UPDATE *parse_op(const char *s) {
    ...
    if(fc && fc[1] && fc[2] == ':') {
        ...
        upd->memstr = static_cast<char *>(memcpy(cfg_malloc(__func__, fc - s + 1), s, fc - s));
        upd->op =
                fc[1]=='r'? DEVICE_READ:
    ...
    if(len > 2 && fn[len-2] == ':') { // Assume format specified
        ...
        if(upd->format == FMT_ERROR) {
            ...
            for(int f, c, i=0; i<62; i++) {
                ...
                f = fileio_format(c);
                if(f != FMT_ERROR)
                    imsg_error("  :%c %s\n", c, fileio_fmtstr(static_cast<FILEFMT>(f)));
            }
            free(upd->memstr);
            free(upd);
            return NULL;
    ...
    upd->filename = static_cast<char *>(memcpy(cfg_malloc(__func__, len + 1), fn, len));
...
char *update_str(const UPDATE *upd) {
    ...
    return str_sprintf("-U %s:%c:%s:%c",
                       ...
                       upd->filename,
                       fileio_fmtchr(static_cast<FILEFMT>(upd->format)));
}
...
int update_dryrun(const AVRPART *p, UPDATE *upd) {
    ...
    if(upd->cmdline) {            // Todo: parse terminal command line?
        termcmds = static_cast<const char **>(realloc(termcmds, sizeof(*termcmds) * (nterms + 1)));
        ...
        if(str_eq(upd->filename, "-")) {
            ...
            if(quell_progress < 2)
                pmsg_notice("%s file %s auto detected as %s\n",
                            ...
                            fileio_fmtstr(static_cast<FILEFMT>(upd->format)));
        }
    }
        ...
        case DEVICE_READ:
            if(upd->format == FMT_IMM) {
                ...
                if(!update_is_writeable(upd->filename)) {
                    ...
                    if(!str_eq(upd->filename, "-") && (wrote = static_cast<const char **>(realloc(wrote, sizeof(*wrote) *
                                                                                                         (nfwritten + 1)))))
...
int do_op(const PROGRAMMER *pgm, const AVRPART *p, const UPDATE *upd, enum updateflags flags) {
    ...
    char *alias_mem_desc = static_cast<char *>(cfg_malloc("do_op()", 2 + (alias_mem && alias_mem->desc ? strlen(
            alias_mem->desc) : 0)));
    ...
    switch (upd->op) {
        case DEVICE_READ:
            ...
            rc = fileio(FIO_WRITE, upd->filename, static_cast<FILEFMT>(upd->format), p, upd->memstr, size);
            if (rc < 0) {
        ...
        case DEVICE_WRITE:
            ...
            rc = fileio(FIO_READ, upd->filename, static_cast<FILEFMT>(upd->format), p, upd->memstr, -1);
            if (rc < 0) {
        ...
        case DEVICE_VERIFY:
            ...
            userverify = upd->op == DEVICE_VERIFY; // Explicit -U :v by user
            ...
            if (userverify) {
                ...
                rc = fileio(FIO_READ_FOR_VERIFY, upd->filename, static_cast<FILEFMT>(upd->format), p, upd->memstr, -1);
            ...
            verified = fs.nbytes+fs.ntrailing;
