...
int fileio_mem(int op, const char *filename, FILEFMT format, const AVRPART *p, const AVRMEM *mem, int msize) {
  ...
  if(str_starts(filename, "urboot:") && (op == FIO_READ || op == FIO_READ_FOR_VERIFY)) {
    ...
    int usage;
    uint16_t *bootloader = urbootautogen(filename, &usage);
    if(bootloader && usage > msize) {
      pmsg_error("unexpected urboot:... bootloader size %d does not fit into flash\n", usage);
      bootloader = NULL;
    }
    if(!bootloader || !usage)
      return -1;
    memcpy(mem->buf+msize-usage, bootloader, usage);
    memset(mem->tags+msize-usage, TAG_ALLOCATED, usage);
