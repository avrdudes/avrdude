...
static FILEFMT couldbe(int first, unsigned char *line) {
    ...
    if(*p && *p != '#') {
        ...
        if(!failed && idx[0]+idx[1]+idx[2]+idx[3]) {
            ...
            return (FILEFMT) fmts[idx[i0] >= idx[i2] ? i0 : i2];
        }
    }
...
static int fileio_segments_normalise(int oprwv, const char *filename, FILEFMT format,
  ...
  if(format == FMT_RBIN || format == FMT_ELF)
  {
    ...
    for(int i=0; i<n; i++) {
        ...
        Segorder_t where = static_cast<Segorder_t>(0);

        if(i == 0)
            //where |= FIRST_SEG;
            where = static_cast<Segorder_t>(where | FIRST_SEG);
        if(i+1 == n)
            //where |= LAST_SEG;
            where = static_cast<Segorder_t>(where | LAST_SEG);
