...
PROGRAMMER *pgm_new(void) {
  ...
  pgm->cp_flash = static_cast<AVR_Cache *>(cfg_malloc("pgm_new()", sizeof(AVR_Cache)));
  pgm->cp_eeprom = static_cast<AVR_Cache *>(cfg_malloc("pgm_new()", sizeof(AVR_Cache)));
  pgm->cp_bootrow = static_cast<AVR_Cache *>(cfg_malloc("pgm_new()", sizeof(AVR_Cache)));
  pgm->cp_usersig = static_cast<AVR_Cache *>(cfg_malloc("pgm_new()", sizeof(AVR_Cache)));
  ...
  pgm->leds = static_cast<leds_t *>(cfg_malloc(__func__, sizeof(leds_t)));
  /*
  ...
  if(src) {
    ...
    if(src->hvupdi_support)
      for(LNODEID ln = lfirst(src->hvupdi_support); ln; ln = lnext(ln)) {
        int *ip = static_cast<int *>(cfg_malloc("pgm_dup()", sizeof(int)));
        *ip = *(int *) ldata(ln);
...
PROGRAMMER *locate_programmer_starts_set(const LISTID programmers, const char *pgid, const char **setid, AVRPART *prt) {
  ...
  for(LNODEID ln1=lfirst(programmers); ln1; ln1=lnext(ln1)) {
    pgm = static_cast<PROGRAMMER *>(ldata(ln1));
...
PROGRAMMER *locate_programmer_set(const LISTID programmers, const char *configid, const char **setid) {
  for(LNODEID ln1=lfirst(programmers); ln1; ln1=lnext(ln1)) {
    PROGRAMMER *p = static_cast<PROGRAMMER *>(ldata(ln1));
...
static int sort_programmer_compare(const PROGRAMMER *p1, const PROGRAMMER *p2) {
  ...
  return strcasecmp((const char *)ldata(lfirst(p1->id)), (const char *)ldata(lfirst(p2->id)));
}
