...
extern "C" {
  ...
  int updi_nvm_erase_user_row_V5(const PROGRAMMER *pgm, const AVRPART *p, uint32_t address,
    uint16_t size);
  ...
  int updi_nvm_write_eeprom_V5(const PROGRAMMER *pgm, const AVRPART *p, uint32_t address,
    unsigned char *buffer, uint16_t size);
  int updi_nvm_write_fuse_V5(const PROGRAMMER *pgm, const AVRPART *p, uint32_t address,
    uint8_t value);
