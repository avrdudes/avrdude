...
int avr_tpi_chip_erase(const PROGRAMMER *pgm, const AVRPART *p) {
    ...
    if (p->prog_modes & PM_TPI) {
        ...
        unsigned char cmd[] = {
                ...
                (TPI_CMD_SSTPR | 0),
                static_cast<unsigned char>(((mem->offset & 0xFF) | 1)),
                /* and low byte */
                (TPI_CMD_SSTPR | 1),
                static_cast<unsigned char>(((mem->offset >> 8) & 0xFF)),
...
int avr_write_mem(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *m, int size, int auto_erase) {
    ...
    if ((pgm->paged_load && m->page_size > 1 && m->size % m->page_size == 0) ||
        ...
        uint8_t *spc = static_cast<uint8_t *>(cfg_malloc(__func__, cm->page_size));
...
int avr_mem_cmp(void *mem1, void *mem2) {
    AVRMEM *m1 = static_cast<AVRMEM *>(mem1), *m2 = static_cast<AVRMEM *>(mem2);
