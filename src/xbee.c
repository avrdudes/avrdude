...
/*
 * After eight seconds the AVR bootloader watchdog will kick in.  But to allow
 * for the possibility of eight seconds upstream and another eight seconds
 * downstream, allow for 16 retries (of roughly one second each).
 */
 ...
 *
 * There is an additional overhead of 3 bytes encapsulation, one "REQUEST"
 * byte, one sequence number byte, and one "FIRMWARE_DELIVER" request type.
 *
 * The ZigBee maximum (unfragmented) payload is 84 bytes.  Source routing
 * decreases that by two bytes overhead, plus two bytes per hop.  Maximum hop
 * support is for 11 or 25 hops depending on firmware.
 *
 * Network layer encryption decreases the maximum payload by 18 bytes. APS
 * end-to-end encryption decreases the maximum payload by 9 bytes. Both these
 * layers are available in concert, as seen in the section "Network and APS
 * layer encryption", decreasing our maximum payload by both 18 bytes and 9
 * bytes.
 *
 * Our maximum payload size should therefore ideally be 84 - 18 - 9 = 57 bytes,
 * and therefore a chunk size of 54 bytes for zero hops.
 *
 * Source: XBee X2C manual: "Maximum RF payload size" section for most details;
 * "Network layer encryption and decryption" section for the reference to 18
 * bytes of overhead; and "Enable APS encryption" for the reference to 9 bytes
 * of overhead.
 */
 ...
 * documentation variously as 40 hops (routing table); OR 25 hops (firmware
 * 4x58 or later); OR 11 hops (firmware earlier than 4x58).
 *
 * What isn't described is how to know if a given source route length is
 * actually supported by the mesh for our target device.
 */
...
// Read signature bytes: direct copy of the Arduino behaviour to satisfy Optiboot
...
struct XBeeBootSession {
  ...
  // Set to non-zero if the transport is broken to the point it is considered unusable
  ...
  /*
   * The source route is an array of intermediate 16 bit addresses, starting
   * with the address nearest to the target address, and finishing with the
   * address closest to our local device.
   */
...
static void xbeeStatsAdd(struct XBeeStaticticsSummary *summary, struct timeval const *sample) {
  ...
  if(summary->samples == 0 ||
    ...
    (summary->minimum.tv_sec == sample->tv_sec && summary->minimum.tv_usec > sample->tv_usec)) {
    summary->minimum = *sample;
  }

  if(summary->maximum.tv_sec < sample->tv_sec ||
    (summary->maximum.tv_sec == sample->tv_sec && summary->maximum.tv_usec < sample->tv_usec)) {
...
static void xbeedev_stats_send(struct XBeeBootSession *xbs,
  char const *detail,
  int detailSequence, unsigned int group, unsigned char sequence, xbee_stat_is_retry retry,
  struct timeval const *sendTime) {

  struct XBeeSequenceStatistics *stats = &xbs->sequenceStatistics[group*256 + sequence];
  ...
  if(detailSequence >= 0) {
    ...
    pmsg_debug("stats: send Group %s Sequence %u : Send %lu.%06lu %s\n",
      groupNames[group], (unsigned int) sequence, (unsigned long) sendTime->tv_sec,
     (unsigned long) sendTime->tv_usec, detail);
  }
}

static void xbeedev_stats_receive(struct XBeeBootSession *xbs,
  char const *detail, unsigned int group, unsigned char sequence, struct timeval const *receiveTime) {
  struct XBeeSequenceStatistics *stats = &xbs->sequenceStatistics[group*256 + sequence];
  ...
  pmsg_debug("stats: receive Group %s Sequence %u : "
    ...
    (unsigned long) receiveTime->tv_usec, (unsigned long) secs, (unsigned long) usecs, detail);
...
static int sendAPIRequest(struct XBeeBootSession *xbs,
  ...
  int appType,
  char const *detail,
  int detailSequence, unsigned int frameGroup, xbee_stat_is_retry retry,
  ...
  pmsg_debug("%s(): %lu.%06lu %d, %d, %d, %d %s\n", __func__,
    ...
    (unsigned long) time.tv_usec, (int) packetType, (int) sequence, appType, data == NULL? -1: (int) *data, detail);
...
#define fpput(x)                                             \
  ...
  if(txSequence >= 0) {
    ...
    // Record the frame send time; note that frame sequences are never retries
    xbeedev_stats_send(xbs, detail, detailSequence, frameGroup, txSequence, 0, &time);
  }
  ...
  if(apiType != 0x08) {
    ...
    /*
     * If this is an API call with remote address, but is not a Create Source
     * Route request, consider prefixing it with source routing instructions.
     */
  ...
  if(sequence >= 0) {
    ...
    if(packetType == XBEEBOOT_PACKET_TYPE_REQUEST)
      xbeedev_stats_send(xbs, detail, sequence, XBEE_STATS_TRANSMIT, sequence, retry, &time);
  }
  ...
  return xbs->serialDevice->send(&xbs->serialDescriptor, frameStart, finalLength + prefixLength);
}

static int sendPacket(struct XBeeBootSession *xbs,
  const char *detail,
  unsigned char packetType,
  unsigned char sequence, xbee_stat_is_retry retry, int appType, unsigned int dataLength, const unsigned char *data) {
  unsigned char apiType;
  int prePayload1;
  int prePayload2;
  ...
  if(xbs->directMode) {
    /*
     * In direct mode we are pretending to be an XBee device forwarding on data
     * received from the transmitting XBee.  We therefore format the data as a
     * remote XBee would, encapsulated in a 0x90 packet.
     */
    apiType = 0x90;             // ZigBee Receive Packet
    prePayload1 = -1;
    prePayload2 = -1;
  } else {
    // In normal mode we are requesting a payload delivery, encapsulated in a 0x10 packet
  ...
  return sendAPIRequest(xbs, apiType, xbs->txSequence, -1,
    prePayload1, prePayload2, packetType, sequence, appType, detail, sequence,
    XBEE_STATS_FRAME_REMOTE, retry, dataLength, data);
}
...
static void xbeedev_record16Bit(struct XBeeBootSession *xbs, const unsigned char *rx16Bit) {
  /*
   * We don't start out knowing what the 16-bit device address is, but we
   * should receive it on the return packets, and re-use it from that point on.
   */
  unsigned char *const tx16Bit = &xbs->xbee_address[XBEE_ADDRESS_64BIT_LEN];

  if(memcmp(rx16Bit, tx16Bit, XBEE_ADDRESS_16BIT_LEN) != 0) {
    pmsg_debug("%s(): new 16-bit address: %02x%02x\n",
      __func__, (unsigned int) rx16Bit[0], (unsigned int) rx16Bit[1]);
...
static int xbeedev_poll(struct XBeeBootSession *xbs, unsigned char **buf, size_t *buflen,
  int waitForAck, int waitForSequence) {
  ...
  start_of_frame:
    {
      ...
      do {
        ...
        if(byte == 0x7e)
          /*
           * No matter when we receive a frame start byte, we should abort
           * parsing and start a fresh frame.
           */
          goto start_of_frame;
        ...
        if(index == XBEE_LENGTH_LEN) {
          ...
          frameSize = (frame[0] << 8 | frame[1]) + XBEE_LENGTH_LEN + XBEE_CHECKSUM_LEN;
    ...
    pmsg_debug("%s(): %lu.%06lu Received frame type %x\n", __func__,
      (unsigned long) receiveTime.tv_sec, (unsigned long) receiveTime.tv_usec, (unsigned int) frameType);
    ...
    if(frameType == 0x97 && frameSize > 16) {
      ...
      xbeedev_stats_receive(xbs, "Remote AT command response", XBEE_STATS_FRAME_REMOTE, txSequence, &receiveTime);

      pmsg_notice("%s(): remote command %d result code %d\n", __func__, (int) txSequence, (int) resultCode);
      ...
      xbeedev_stats_receive(xbs, "Local AT command response", XBEE_STATS_FRAME_LOCAL, txSequence, &receiveTime);

      pmsg_notice("%s(): local command %c%c result code %d\n", __func__, frame[4], frame[5], (int) frame[6]);
      ...
      xbeedev_stats_receive(xbs, "Transmit status", XBEE_STATS_FRAME_REMOTE, txSequence, &receiveTime);

      pmsg_debug("%s(): transmit status %d result code %d\n", __func__, (int) frame[3], (int) frame[7]);
    } else if(frameType == 0xa1 &&
      frameSize >= XBEE_LENGTH_LEN + XBEE_APITYPE_LEN + XBEE_ADDRESS_64BIT_LEN +
      XBEE_ADDRESS_16BIT_LEN + 2 + XBEE_CHECKSUM_LEN) {

      // Route Record Indicator
      if(memcmp(&frame[XBEE_LENGTH_LEN + XBEE_APITYPE_LEN], xbs->xbee_address, XBEE_ADDRESS_64BIT_LEN) != 0) {
      ...
      /*
       * We don't start out knowing what the 16-bit device address is, but we
       * should receive it on the return packets, and re-use it from that point
       * on.
       */
      {
        const unsigned char *rx16Bit = &frame[XBEE_LENGTH_LEN + XBEE_APITYPE_LEN + XBEE_ADDRESS_64BIT_LEN];

        xbeedev_record16Bit(xbs, rx16Bit);
      }

      const unsigned int header = XBEE_LENGTH_LEN + XBEE_APITYPE_LEN + XBEE_ADDRESS_64BIT_LEN + XBEE_ADDRESS_16BIT_LEN;
      ...
      if(hops <= XBEE_MAX_INTERMEDIATE_HOPS) {
        if(xbs->sourceRouteHops != (int) hops || memcmp(&frame[tableOffset], xbs->sourceRoute, hops*2) != 0) {
      ...
      if(frameType == 0x10) {
        ...
        const unsigned int header = XBEE_LENGTH_LEN +
          ...
          XBEE_ADDRESS_64BIT_LEN + XBEE_ADDRESS_16BIT_LEN + XBEE_RADIUS_LEN + XBEE_TXOPTIONS_LEN;
        ...
        if(memcmp(&frame[XBEE_LENGTH_LEN + XBEE_APITYPE_LEN], xbs->xbee_address, XBEE_ADDRESS_64BIT_LEN) != 0) {
          /*
           * This packet is not from our target device.  Unlikely to ever
           * happen, but if it does we have to ignore it.
           */
          continue;
        }
        ...
        /*
         * We don't start out knowing what the 16-bit device address is, but we
         * should receive it on the return packets, and re-use it from that
         * point on.
         */
        {
          const unsigned char *rx16Bit = &frame[XBEE_LENGTH_LEN + XBEE_APITYPE_LEN + XBEE_ADDRESS_64BIT_LEN];
      ...
      if(dataLength >= 2) {
        ...
        if(protocolType == XBEEBOOT_PACKET_TYPE_ACK) {
          // ACK
          xbeedev_stats_receive(xbs, "XBeeBoot ACK", XBEE_STATS_TRANSMIT, sequence, &receiveTime);

          // We can't update outSequence here, we already do that somewhere else
          if(waitForAck >= 0 && waitForAck == sequence)
            return 0;
        } else if(protocolType == XBEEBOOT_PACKET_TYPE_REQUEST && dataLength >= 4 && dataStart[2] == 24) {
          // REQUEST FRAME_REPLY
          xbeedev_stats_receive(xbs, "XBeeBoot Receive", XBEE_STATS_RECEIVE, sequence, &receiveTime);
          ...
          if(sequence == nextSequence) {
            ...
            sendPacket(xbs, "Transmit Request ACK for RECEIVE",
              XBEEBOOT_PACKET_TYPE_ACK, sequence, XBEE_STATS_NOT_RETRY, -1, 0, NULL);
             ...
             * Not a retry, this is the first point we know for sure for this
             * sequence number.
             */
            while((++nextSequence & 0xff) == 0);
            xbeedev_stats_send(xbs, "poll() implies pending RECEIVE",
              nextSequence, XBEE_STATS_RECEIVE, nextSequence, XBEE_STATS_NOT_RETRY, &receiveTime);
          }
        }
      }
    }
 ...
 *   value indicating the sequence number associated with the request
 */
static int localAsyncAT(struct XBeeBootSession *xbs, char const *detail,
  ...
  if(xbs->directMode)
    /*
     * Remote XBee AT commands make no sense in direct mode - there is no XBee
...
static int localAT(struct XBeeBootSession *xbs, char const *detail, unsigned char at1, unsigned char at2, int value) {
  int result = localAsyncAT(xbs, detail, at1, at2, value);

  if(result <= 0)               // Failure, or success without a sequence number
    return result;
 ...
 * Return -512 + XBee AT Response code
 */
static int sendAT(struct XBeeBootSession *xbs, char const *detail, unsigned char at1, unsigned char at2, int value) {
  if(xbs->directMode)
    /*
     * Remote XBee AT commands make no sense in direct mode - there is no XBee
     * device to communicate with.
  ...
  sendAPIRequest(xbs, 0x17, sequence, -1,
    -1, -1, -1, 0x02, -1, detail, -1, XBEE_STATS_FRAME_REMOTE, XBEE_STATS_NOT_RETRY, length, buf);
  ...
  for(retries = 0; retries < 30; retries++) {
    ...
    if(xbeeRc == 0)             // Translate to normal success code
      return 0;
    if(rc != -1)
      return rc;
  }
...
static int xbeedev_open(const char *port, union pinfo pinfo, union filedescriptor *fdp) {
  /*
  ...
  pmsg_trace("XBee address: %02x%02x%02x%02x%02x%02x%02x%02x\n",
    ...
    (unsigned int) xbs->xbee_address[5], (unsigned int) xbs->xbee_address[6], (unsigned int) xbs->xbee_address[7]);
  ...
  if(pinfo.serialinfo.baud) {
     ...
     * Why?
     *
     * XBee devices default to 9600 baud.  In this mode we are talking to the
     * XBee device, not the far-end device, so it's the local XBee baud rate we
     * should select.  The baud rate of the AVR device is irrelevant.
     */
  ...
  if(!xbs->directMode) {
    ...
    /*
     * At this point we want to set the remote XBee parameters as required for
     * talking to XBeeBoot.  Ideally we would start with an "FR" full reset,
     * but because that causes the XBee to disappear off the mesh for a
     * significant period and become unresponsive, we don't do that.
     */

    /*
     * Issue an "Aggregate Routing Notification" to enable many-to-one routing
     * to this device.  This has two effects:
     *
     * - Establishes a route from the remote XBee attached to the CPU being
     *   programmed back to the local XBee.
     *
     * - Enables the 0xa1 Route frames so that we can make use of Source
     *   Routing to deliver packets directly to the remote XBee.
     *
     * Under "RF packet routing" subsection "Many-to-One routing", the XBee S2C
     * manual states "Applications that require multiple data collectors can
     * also use many-to-one routing. If more than one data collector device
     * sends a many-to-one broadcast, devices create one reverse routing table
     * entry for each collector."
     *
     * Under "RF packet routing" subsection "Source routing", the XBee S2C
     * manual states "To use source routing, a device must use the API mode,
     * and it must send periodic many-to-one route request broadcasts (AR
     * command) to create a many-to-one route to it on all devices".
     */
    {
    ...
    /*
     * Disable RTS input on the remote XBee, just in case it is enabled by
     * default.  XBeeBoot doesn't attempt to support flow control, and so it
     * may not correctly drive this pin if RTS mode is the default
     * configuration.
     *
...
static int xbeedev_send(const union filedescriptor *fdp, const unsigned char *buf, size_t buflen) {
  struct XBeeBootSession *xbs = xbeebootsession(fdp);

  if(xbs->transportUnusable)    // Don't attempt to continue on an unusable transport layer
    return -1;
  ...
  while(buflen > 0) {
    ...
    /*
     * We are about to send some data, and that might lead potentially to
     * received data before we see the ACK for this transmission. As this might
     * be the trigger seen before the next "recv" operation, record that we
     * have delivered this potential trigger.
     */
    {
      unsigned char nextSequence = xbs->inSequence;
      ...
      /*
       * Optimistic records should never be treated as retries, because they
       * might simply be guessing too optimistically.
       */
      xbeedev_stats_send(xbs, "send() hints possible triggered RECEIVE",
        nextSequence, XBEE_STATS_RECEIVE, nextSequence, 0, &sendTime);
    }
    ...
    /*
     * Source routing incurs a two byte fixed overhead, plus a two byte
     * additional cost per intermediate hop.
     *
     * We are attempting to avoid fragmentation here, so resize our maximum
     * size to anticipate the overhead of the current number of hops.  If our
     * maximum chunk would be less than one, just give up and hope
     * fragmentation will somehow save us.
     */
    ...
    const unsigned char blockLength = (buflen > maximum_chunk)? maximum_chunk: buflen;
    ...
    for(retries = 0; retries < XBEE_MAX_RETRIES; retries++) {
      ...
      /*
       * Test the connection to the local XBee by repeatedly requesting local
       * configuration details.  This functionally has no effect, but will
       * allow us to measure any reliability issues on this link.
       */
      localAsyncAT(xbs, "Local XBee ping [send]", 'A', 'P', -1);

      /*
       * If we don't receive an ACK it might be because the chip missed an ACK
       * from us.  Resend that too after a timeout, unless it's zero which is
       * an illegal sequence number.
       */
      if(xbs->inSequence != 0) {
        int ackRc = sendPacket(xbs,
          "Transmit Request ACK [Retry in send] " "for RECEIVE",
...
static int xbeedev_recv(const union filedescriptor *fdp, unsigned char *buf, size_t buflen) {
  struct XBeeBootSession *xbs = xbeebootsession(fdp);

  /*
   * First de-buffer anything previously received in a chunk that couldn't be
   * immediately delievered.
   */
  ...
  // When we expect to receive data, that is the time to start the clock
  {
    unsigned char nextSequence = xbs->inSequence;
    ...
    // Not a retry - in fact this is the first stage we know for sure a RECEIVE is due
    xbeedev_stats_send(xbs, "recv() implies pending RECEIVE",
      nextSequence, XBEE_STATS_RECEIVE, nextSequence, XBEE_STATS_NOT_RETRY, &sendTime);
  }
  ...
  for(retries = 0; retries < XBEE_MAX_RETRIES; retries++) {
    ...
    /*
     * Test the connection to the local XBee by repeatedly requesting local
     * configuration details.  This functionally has no effect, but will allow
     * us to measure any reliability issues on this link.
     */
    localAsyncAT(xbs, "Local XBee ping [recv]", 'A', 'P', -1);

    // The chip may have missed an ACK from us; resend after a timeout
    if(xbs->inSequence != 0)
      sendPacket(xbs, "Transmit Request ACK [Retry in recv] for RECEIVE",
        XBEEBOOT_PACKET_TYPE_ACK, xbs->inSequence, XBEE_STATS_IS_RETRY, -1, 0, NULL);
  }
  return -1;
}
...
static int xbeedev_set_dtr_rts(const union filedescriptor *fdp, int is_on) {
  ...
  if(xbs->directMode)           // Correct for direct mode
    return xbs->serialDevice->set_dtr_rts(&xbs->serialDescriptor, is_on);

  /*
   * For non-direct mode (Over-The-Air) we need to issue XBee commands to the
   * remote XBee in order to reset the AVR CPU and initiate the XBeeBoot
   * bootloader.
   */
...
static int xbee_getsync(const PROGRAMMER *pgm) {
  ...
  /*
   * Issue sync request as per STK500.  Unlike stk500_getsync(), don't retry
   * here - the underlying protocol will deal with retries for us in
   * xbeedev_send() and should be reliable.
   */
  ...
  // The same is true of the receive - it will retry on timeouts until the response buffer is full
...
static int xbee_open(PROGRAMMER *pgm, const char *port) {
  ...
  /*
   * At this point stk500_drain() and stk500_getsync() calls would normally be
   * made.  But given that we have a transport layer over the serial command
   * stream, the drain and repeated STK_GET_SYNC requests are not very helpful.
   * Instead, skip the draining entirely, and issue the STK_GET_SYNC ourselves.
   */
...
static void xbee_close(PROGRAMMER *pgm) {
  ...
  /*
   * We have tweaked a few settings on the XBee, including the RTS mode and the
   * reset pin's configuration.  Do a soft full reset, restoring the device to
   * its normal power-on settings.
   *
...
static int xbee_parseextparms(const PROGRAMMER *pgm, const LISTID extparms) {
  ...
  for(LNODEID ln = lfirst(extparms); ln; ln = lnext(ln)) {
    ...
    if(str_starts(extended_param, "xbeeresetpin=")) {
      ...
      if(sscanf(extended_param, "xbeeresetpin=%i", &resetpin) != 1 || resetpin <= 0 || resetpin > 7) {
