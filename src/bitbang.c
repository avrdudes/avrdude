...
static void bitbang_calibrate_delay(void) {
...
#if defined(WIN32)
  /*
   * If the hardware supports a high-resolution performance counter, we
   * ultimately prefer that one, as it gives quite accurate delays on modern
   * high-speed CPUs.
   */
  if(QueryPerformanceFrequency(&freq)) {
    ...
    /*
     * If a high-resolution performance counter is not available, we don't have
     * any Win32 implementation for setting up the per-microsecond delay count,
     * so we can only run on a preconfigured delay stepping there.  The figure
     * below should at least be correct within an order of magnitude, judging
     * from the auto-calibration figures seen on various Unix systems on
     * comparable hardware.
     */
  ...
  *(volatile int *) &cx->bb_done = 0;
  ...
  /*
   * Set ITIMER_REAL to 100 ms.  All known systems have a timer granularity of
   * 10 ms or better, so counting the delay cycles accumulating over 100 ms
   * should give us a rather realistic picture, without annoying the user by a
   * lengthy startup time (as an alarm(1) would do).  Of course, if heavy
   * system activity happens just during calibration but stops before the
   * remaining part of AVRDUDE runs, this will yield wrong values.  There's not
...
/*
 * Delay for approximately the number of microseconds specified. usleep()'s
 * granularity is usually like 1 ms or 10 ms, so it's not really suitable for
 * short delays in bit-bang algorithms.
 */
...
static unsigned char bitbang_txrx(const PROGRAMMER *pgm, unsigned char byte) {
  ...
  for(i = 7; i >= 0; i--) {
    ...
    // The result bit is either valid from a previous falling edge or ignored in current context
...
// Transmit bytes via SPI and return the results; 'cmd' and 'res' must point to data buffers
...
int bitbang_initialize(const PROGRAMMER *pgm, const AVRPART *p) {
  ...
  /*
   * Enable programming mode.  If we are programming an AT90S1200, we can only
   * issue the command and hope it worked.  If we are using one of the other
   * chips, the chip will echo 0x53 when issuing the third byte of the command.
   * In this case, try up to 32 times in order to possibly get back into sync
   * with the chip if we are out of sync.
   */
