...
static const char *pgmname(const PROGRAMMER *pgm) {
    ...
    const char *name[] = { // Parallel to pdata::PGMTYPE_ definitions in stk500v2_private.h
            "unknown",
            "STK500",
            "AVRISP",
            "AVRISP mkII",
    ...
    return i==pdata::PGMTYPE_STK500 && PDATA(pgm)->is_scratchmonkey? "SCRATCHMONKEY": // STK500 with LEDs
...
static int stk500v2_send(const PROGRAMMER *pgm, unsigned char *data, size_t len) {
    ...
    if (PDATA(pgm)->pgmtype == pdata::PGMTYPE_AVRISP_MKII ||
        PDATA(pgm)->pgmtype == pdata::PGMTYPE_STK600)
        return stk500v2_send_mk2(pgm, data, len);
    else if (PDATA(pgm)->pgmtype == pdata::PGMTYPE_JTAGICE_MKII)
        return stk500v2_jtagmkII_send(pgm, data, len);
    else if (PDATA(pgm)->pgmtype == pdata::PGMTYPE_JTAGICE3)
...
int stk500v2_getsync(const PROGRAMMER *pgm) {
    ...
    if (PDATA(pgm)->pgmtype == pdata::PGMTYPE_JTAGICE_MKII ||
        PDATA(pgm)->pgmtype == pdata::PGMTYPE_JTAGICE3)
        return 0;
...
static int stk500v2_program_enable(const PROGRAMMER *pgm, const AVRPART *p) {
    ...
    if (PDATA(pgm)->pgmtype == pdata::PGMTYPE_STK500 ||
        PDATA(pgm)->pgmtype == pdata::PGMTYPE_STK600)
    ...
    if (rv < 0) {
        ...
        {
            case pdata::PGMTYPE_STK600:
            case pdata::PGMTYPE_AVRISP_MKII:
            ...
            default:
                /* cannot report anything for other pdata::PGMTYPEs */
                break;
        }
    }
...
static int stk500hvsp_program_enable(const PROGRAMMER *pgm, const AVRPART *p) {
    ...
    buf[0] = PDATA(pgm)->pgmtype == pdata::PGMTYPE_STK600?
...
static int stk500v2_initialize(const PROGRAMMER *pgm, const AVRPART *p) {
    ...
    if ((PDATA(pgm)->pgmtype == pdata::PGMTYPE_STK600 ||
         PDATA(pgm)->pgmtype == pdata::PGMTYPE_AVRISP_MKII ||
         PDATA(pgm)->pgmtype == pdata::PGMTYPE_JTAGICE_MKII) != 0
    ...
    if (PDATA(pgm)->varef_get || PDATA(pgm)->varef_set) {
        ...
        } else if(PDATA(pgm)->pgmtype == pdata::PGMTYPE_STK600) {
    ...
    if (PDATA(pgm)->fosc_get || PDATA(pgm)->fosc_set) {
        if(PDATA(pgm)->pgmtype == pdata::PGMTYPE_STK500 || PDATA(pgm)->pgmtype == pdata::PGMTYPE_STK600) {
...
static int stk500v2_open(PROGRAMMER *pgm, const char *port) {
    ...
    PDATA(pgm)->pgmtype = pdata::PGMTYPE_UNKNOWN;
...
static double stk500v2_vtarget_value(const PROGRAMMER *pgm) {
    ...
    if (PDATA(pgm)->pgmtype != pdata::PGMTYPE_JTAGICE3) {
    ...
    switch (PDATA(pgm)->pgmtype) {
        ...
        case pdata::PGMTYPE_JTAGICE3:
        {
            ...
            for(LNODEID ln=lfirst(pgm->id); ln; ln=lnext(ln))
                ladd(pgmcp->id, cfg_strdup("stk500v2_display()", (const char *)ldata(ln)));
...
static int stk500v2_jtagmkII_open(PROGRAMMER *pgm, const char *port) {
   ...
   */
    ...
    PDATA(pgm)->pgmtype = pdata::PGMTYPE_JTAGICE_MKII;
