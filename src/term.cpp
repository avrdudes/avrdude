...
static int cmd_dump(const PROGRAMMER *pgm, const AVRPART *p, int argc, const char *argv[]) {
    ...
    uint8_t *buf = static_cast<uint8_t *>(malloc(read_mem[i].len));
    if (buf == NULL) {
...
static int cmd_write(const PROGRAMMER *pgm, const AVRPART *p, int argc, const char *argv[]) {
    ...
    unsigned char *buf = static_cast<unsigned char *>(calloc(bufsz, 1)), *tags = static_cast<unsigned char *>(calloc(
            bufsz, 1));
...
static int cmd_save(const PROGRAMMER *pgm, const AVRPART *p, int argc, const char *argv[]) {
    ...
    char *filename = static_cast<char *>(memcpy(cfg_malloc(__func__, len + 1), fn, len));

    mem = avr_dup_mem(omem);
    int n = argc > 3? (argc-3)/2: 1;
    Segment_t *seglist = static_cast<Segment_t *>(cfg_malloc(__func__, n * sizeof *seglist));
...
static int cmd_config(const PROGRAMMER *pgm, const AVRPART *p, int argc, const char *argv[]) {
    ...
    int ret = 0;
    cc = static_cast<Cfg_t *>(cfg_malloc(__func__, sizeof *cc * nc));
    fc = static_cast<Flock_t *>(cfg_malloc(__func__, sizeof *fc * nc));
...
static int cmd_factory(const PROGRAMMER *pgm, const AVRPART *p, int argc, const char *argv[]) {
    ...
    for(LNODEID ln=lfirst(p->mem); ln; ln=lnext(ln))
        if(mem_is_a_fuse(m = static_cast<AVRMEM *>(ldata(ln))))
    ...
    for(LNODEID ln=lfirst(p->mem); ln; ln=lnext(ln))
        if(mem_is_lock(m = static_cast<AVRMEM *>(ldata(ln))))
            if(fusel_factory(pgm, p, m) < 0)
                ret = -1;
...
static char *tokenize(char *s, int *argcp, const char ***argvp) {
    ...
    argv = static_cast<const char **>(cfg_malloc(__func__, (nargs + 2) * sizeof(char *) + slen + nargs));
