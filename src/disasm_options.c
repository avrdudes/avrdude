...
char Options_ParseCmdLine(struct Options *Options, int argc, char **argv) {
  ...
  for(i = 1; i < argc; i++) {
    ...
    if(StringStart(Argument, "-a") == 0) {
      if(SetVal(Argument + 2, &(Options->Show_Addresses), "show addresses", 0, 1) == 0)
        return 0;
    } else if(StringStart(Argument, "-o") == 0) {
      if(SetVal(Argument + 2, &(Options->Show_Opcodes), "show opcodes", 0, 1) == 0)
        return 0;
    } else if(StringStart(Argument, "-c") == 0) {
      if(SetVal(Argument + 2, &(Options->Show_Comments), "show comments", 0, 1) == 0)
        return 0;
    } else if(StringStart(Argument, "-q") == 0) {
      if(SetVal(Argument + 2, &(Options->Show_Cycles), "show cycles", 0, 1) == 0)
        return 0;
    } else if(StringStart(Argument, "-p") == 0) {
      if(SetVal(Argument + 2, &(Options->Show_PseudoCode), "show pseudocode", 0, 1) == 0)
        return 0;
    } else if(StringStart(Argument, "-s") == 0) {
      if(SetVal(Argument + 2, &(Options->CodeStyle), "codestyle", 0, 2) == 0)
        return 0;
    } else if(StringStart(Argument, "-l") == 0) {
      if(SetVal(Argument + 2, &(Options->Process_Labels), "process labels", 0, 1) == 0)
        return 0;
    } else if(StringStart(Argument, "-m") == 0) {
      if(SetMCU(Argument + 2, Options->MCU) == 0)
        return 0;
    } else if(StringStart(Argument, "-t") == 0) {
      strncpy(Options->Tagfile, Argument + 2, 256);
    } else if(StringStart(Argument, "-f") == 0) {
      if(SetValInt(Argument + 2, &(Options->FlashSize), "flash size", 32, 1024 * 1024) == 0)
        return 0;
  ...
  if(Options->Tagfile[0] != 0) {
    if(!Read_Tagfile(Options->Tagfile))
      return 0;
  }
