...
int avr_read_page_default(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *mem, int addr, unsigned char *buf) {
  ...
  unsigned char *pagecopy = static_cast<unsigned char *>(cfg_malloc(__func__, pgsize));
...
static int initCache(AVR_Cache *cp, const PROGRAMMER *pgm, const AVRPART *p) {
  ...
  cp->cont = static_cast<unsigned char *>(cfg_malloc("initCache()", cp->size));
  cp->copy = static_cast<unsigned char *>(cfg_malloc("initCache()", cp->size));
  cp->iscached = static_cast<unsigned char *>(cfg_malloc("initCache()", cp->size / cp->page_size));
