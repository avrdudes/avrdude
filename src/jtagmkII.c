...
#define gotoerr {lineno = __LINE__; goto eRR;}

struct pdata {
  unsigned short command_sequence;      // Next cmd seqno to issue

  // See jtagmkII_read_byte() for an explanation of the flash and EEPROM page caches
...
static int jtagmkII_setparm(const PROGRAMMER *pgm, unsigned char parm, unsigned char *value);
  ...
  case RSP_ILLEGAL_EMULATOR_MODE:
    ...
    if(len > 1)
      ...
      case EMULATOR_MODE_DEBUGWIRE:
        msg_info(": DebugWire");
        break;
      case EMULATOR_MODE_JTAG:
        msg_info(": JTAG");
        break;
      case EMULATOR_MODE_HV:
        msg_info(": HVSP/PP");
        break;
      case EMULATOR_MODE_SPI:
        msg_info(": SPI");
        break;
      case EMULATOR_MODE_JTAG_XMEGA:
        msg_info(": JTAG/Xmega");
        break;
      }
    msg_info("\n");
 ...
 */
static int jtagmkII_recv_frame(const PROGRAMMER *pgm, unsigned char **msg, unsigned short *seqno) {
  enum states { sSTART,
...
retry:
  ...
  if(jtagmkII_setparm(pgm, PAR_EMULATOR_MODE, buf) < 0) {
    if(mode == EMULATOR_MODE_SPI) {
      ...
      if(jtagmkII_setparm(pgm, PAR_EMULATOR_MODE, buf) < 0)
        ...
        /*
         * We are supposed to send a CMND_RESET with the MONCOM_DISABLE flag
         * set right now, and then restart from scratch.
         *
         * As this will make the ICE sign off from USB, so we risk losing our
         * USB connection, it's easier to instruct the user to restart AVRDUDE
         * rather than trying to cope with all this inside the program.
         */
  ...
  // GET_SYNC forces the target into STOPPED mode
  buf[0] = CMND_GET_SYNC;
...
static int jtagmkII_chip_erase(const PROGRAMMER *pgm, const AVRPART *p) {
  ...
  pmsg_notice2("%s(): sending %schip erase command: ", __func__, p->prog_modes & (PM_PDI | PM_UPDI)? "Xmega ": "");
...
static void jtagmkII_set_devdescr(const PROGRAMMER *pgm, const AVRPART *p) {
  ...
  sendbuf.dd.ucAllowFullPageBitstream = (p->flags & AVRPART_ALLOWFULLPAGEBITSTREAM) != 0;
  sendbuf.dd.EnablePageProgramming = (p->flags & AVRPART_ENABLEPAGEPROGRAMMING) != 0;
...
static int jtagmkII_reset(const PROGRAMMER *pgm, unsigned char flags) {
  ...
  /*
   * In debugWire mode, don't reset.  Do a forced stop, and tell the ICE to
   * stop any timers, too.
   */
  ...
  pmsg_notice2("%s(): sending %s command: ", __func__, (pgm->flag & PGM_FL_IS_DW)? "stop": "reset");
...
static int jtagmkII_initialize(const PROGRAMMER *pgm, const AVRPART *p) {
  ...
  /*
   * If this is an ATxmega device in JTAG mode, change the emulator mode from
   * JTAG to JTAG_XMEGA.
   */
  ...
  if(my.fwver >= 0x700 && (p->prog_modes & (PM_PDI | PM_UPDI))) {
     ...
     *
     * Firmware version 7.24 (at least) on the Dragon behaves very strange when
     * it gets a RESET request here.  All subsequent responses are completely
     * off, so the emulator becomes unusable. This appears to be a firmware bug
     * (earlier versions, at least 7.14, didn't experience this), but by
     * omitting the RESET for Xmega devices, we can work around it.
     */
  } else {
...
static void jtagmkII_disable(const PROGRAMMER *pgm) {
   ...
   * not in programming mode, so just call it unconditionally here.
   */
...
static int jtagmkII_page_erase(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *m, unsigned int addr) {
  unsigned char cmd[6];
  unsigned char *resp;
  int status, tries;
  ...
  /*
   * Don't use jtagmkII_memaddr() here.  While with all other commands,
   * firmware 7+ doesn't require the NVM offsets being applied, the erase page
...
static int jtagmkII_read_byte(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *mem,
  ...
  if(mem_is_in_flash(mem)) {
    ...
    if(pgm->flag & PGM_FL_IS_DW) {
      /*
       * In debugWire mode, there is no accessible memory area to read the
       * signature from, but the essential two bytes can be read as a parameter
       * from the ICE.
       */
...
/*
 * Read an emulator parameter.  As the maximal parameter length is 4 bytes by
 * now, we always copy out 4 bytes to *value, so the caller must have allocated
 * sufficient space.
 */
int jtagmkII_getparm(const PROGRAMMER *pgm, unsigned char parm, unsigned char *value) {
  int status;
  unsigned char buf[2], *resp, c;
...
static int jtagmkII_setparm(const PROGRAMMER *pgm, unsigned char parm, unsigned char *value) {
  ...
  /*
   * As the maximal parameter length is 4 bytes, we use a fixed-length buffer,
   * as opposed to malloc()ing it.
   */
  ...
  case PAR_OCD_VTARGET:
    ...
    parstr = "ocd_vtarget";
    break;
  case PAR_OCD_JTAG_CLK:
    size = 1;
    parstr = "ocd_jtag_clk";
    break;
  case PAR_TIMERS_RUNNING:
    size = 1;
    parstr = "timers_running";
    break;
  case PAR_EXTERNAL_RESET:
    size = 1;
    parstr = "external_reset";
    break;
  case PAR_DAISY_CHAIN_INFO:
    size = 4;
    parstr = "daisy_chain_info";
    break;
  case PAR_PDI_OFFSET_START:
    size = 4;
    parstr = "pdi_offset_start";
    break;
  case PAR_PDI_OFFSET_END:
    size = 4;
    parstr = "pdi_offset_end";
    break;
  default:
  ...
  pmsg_notice2("%s() sending set parameter command " "(parm %s 0x%02x, %u bytes): ",
    __func__, parstr, parm, (unsigned) size);
...
static void jtagmkII_display(const PROGRAMMER *pgm, const char *p) {
  ...
  if(jtagmkII_getparm(pgm, PAR_HW_VERSION, hw) < 0 || jtagmkII_getparm(pgm, PAR_FW_VERSION, fw) < 0)
    return;
...
static int jtagmkII_avr32_reset(const PROGRAMMER *pgm, unsigned char val, unsigned char ret1, unsigned char ret2) {
  int status;
...
static int jtagmkII_initialize32(const PROGRAMMER *pgm, const AVRPART *p) {
  ...
  for(j = 0; j < 2; ++j) {
    ...
    if(status != 5 || resp[2] != p->signature[0] || resp[3] != p->signature[1] || resp[4] != p->signature[2]) {
      if(ovsigck) {
...
static unsigned long jtagmkII_read_SABaddr(const PROGRAMMER *pgm, unsigned long addr, unsigned int prefix) {
  unsigned char buf[6], *resp;
  int status;
  unsigned long val;
...
static int jtagmkII_write_SABaddr(const PROGRAMMER *pgm, unsigned long addr, unsigned int prefix, unsigned long val) {
...
static int jtagmkII_flash_lock32(const PROGRAMMER *pgm, unsigned char lock, unsigned int page) {
  ...
  for(i = 0; i < 256; ++i) {
    ...
    if(val == ERROR_SAB)
      continue;
    if(val & AVR32_FLASHC_FSR_RDY)
      break;
  }
  if(val == ERROR_SAB)
    gotoerr;
  if(!(val & AVR32_FLASHC_FSR_RDY))
    gotoerr;                    // Flash better be ready
