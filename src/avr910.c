...
static int avr910_initialize(const PROGRAMMER *pgm, const AVRPART *p) {
  ...
  // Get the programmer identifier; Programmer returns exactly 7 chars without nul
  ...
  if(my.devcode == 0) {
    ...
    // If the user forced the selection, use the first device type that is supported by the programmer
 ...
 */
static int avr910_cmd(const PROGRAMMER *pgm, const unsigned char *cmd, unsigned char *res) {
  char buf[5];
...
static int avr910_parseextparms(const PROGRAMMER *pgm, const LISTID extparms) {
  ...
  for(LNODEID ln = lfirst(extparms); ln; ln = lnext(ln)) {
    ...
    if(str_starts(extended_param, "devcode=")) {
      ...
      if(sscanf(extended_param, "devcode=%i", &devcode) != 1 || devcode <= 0 || devcode > 255) {
...
static int avr910_paged_write_flash(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *m,
  ...
  // If we didn't send the page wr cmd after the last byte written in the loop, send it now
...
static int avr910_paged_write(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *m,
  ...
  if(my.use_blockmode == 1) {
    ...
    while(addr < max_addr) {
      ...
      if(avr910_send(pgm, cmd, 4 + blocksize) < 0 || avr910_vfy_cmd_sent(pgm, "write block") < 0) {
        mmt_free(cmd);
        return -1;
      }
