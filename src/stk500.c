 ...
 */
static int stk500_cmd(const PROGRAMMER *pgm, const unsigned char *cmd, unsigned char *res) {
...
static int stk500_set_extended_parms(const PROGRAMMER *pgm, int n, unsigned char *cmd) {
  unsigned char buf[16];
  int tries = 0;
  int i;
...
/*
 * Crossbow MIB510 initialization and shutdown.  Use cmd = 1 to initialize and
 * cmd = 0 to close.
 */
...
retry:
  ...
  if(n_extparms) {
    if((p->pagel == 0) || (p->bs2 == 0)) {
      ...
      buf[2] = 0xD7;            // They look somehow possible, don't they? ;)
      buf[3] = 0xA0;
    } else {
      buf[2] = p->pagel;
      buf[3] = p->bs2;
...
static int stk500_set_varef(const PROGRAMMER *pgm, unsigned int chan, double v) {
  unsigned utarg = 0;
...
static int stk500_get_varef(const PROGRAMMER *pgm, unsigned int chan, double *v) {
  unsigned uaref = 0;
  int rv;
...
/*
 * This code assumes that each count of the SCK duration parameter represents
 * 8/f, where f is the clock frequency of the STK500 controller processors (not
 * the target).  This number comes from Atmel application note AVR061.  It
 * appears that the STK500 bit bangs SCK. For small duration values, the actual
 * SCK width is larger than expected.  As the duration value increases, the SCK
 * width error diminishes.
 */
