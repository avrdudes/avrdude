...
int avrdude_message2(FILE *fp, int lno, const char *file, const char *func, int msgmode, int msglvl, const char *format, ...) {
    ...
    if ((quell_progress < 2 || fp != stderr? verbose: verbose+1-quell_progress) >= msglvl) {
        ...
        char *p = (char *) cfg_malloc(__func__, rc);
        va_start(ap, format);
...
static int cmp_pgmid(const void *a, const void *b) {
    const pgm_distance *pa = (const pgm_distance *) a, *pb = (const pgm_distance *) b;
...
static int suggest_programmers(const char *programmer, LISTID programmers) {
    ...
    for(LNODEID ln1 = lfirst(programmers); ln1; ln1 = lnext(ln1)) {
        PROGRAMMER *pgm = (PROGRAMMER *) ldata(ln1);
        if(is_programmer(pgm))
            for(LNODEID ln2 = lfirst(pgm->id); ln2; ln2 = lnext(ln2))
                nid++;
    }

    pgm_distance *d = (pgm_distance *) cfg_malloc(__func__, nid * sizeof *d);
    ...
    for(LNODEID ln1 = lfirst(programmers); ln1; ln1 = lnext(ln1)) {
        ...
        for(LNODEID ln2 = lfirst(pgm->id); ln2; ln2 = lnext(ln2)) {
            if(idx < nid) {
                d[idx].pgmid = (const char *) ldata(ln2);
                d[idx].desc = pgm->desc;
...
static void programmer_not_found(const char *programmer, PROGRAMMER *pgm, int pmode) {
    ...
    for(LNODEID ln1=lfirst(programmers); ln1; ln1=lnext(ln1)) {
        PROGRAMMER *pg = (PROGRAMMER *) ldata(ln1);
...
int main(int argc, char * argv [])
{
    ...
    enum updateflags uflags = (enum updateflags) (UF_AUTO_ERASE | UF_VERIFY); /* Flags for do_op() */
    ...
    while ((ch = getopt(argc,argv,"?Ab:B:c:C:DeE:Fi:l:nNp:OP:qrstT:U:uvVx:yY:")) != -1) {
        ...
        switch (ch) {
            ...
            case 'D': /* disable auto erase */
                ...
                uflags = (enum updateflags) (uflags & ~UF_AUTO_ERASE);
                /* fall through */
            ...
            case 'n':
                //uflags |= UF_NOWRITE;
                uflags = (enum updateflags) (uflags | UF_NOWRITE);
                break;
            ...
            case 'V':
                //uflags &= ~UF_VERIFY;
                uflags = (enum updateflags) (uflags & ~UF_VERIFY);
                break;
...
//    }
    ...
    for(LNODEID ln1 = lfirst(part_list); ln1; ln1 = lnext(ln1))
        if((p = (struct avrpart *) ldata(ln1))->mem)
    ...
    for(LNODEID ln1 = lfirst(part_list); ln1; ln1 = lnext(ln1)) {
        AVRPART *p = (AVRPART *) ldata(ln1);
        for(LNODEID ln2 = lfirst(programmers); ln2; ln2 = lnext(ln2)) {
            PROGRAMMER *pgm = (PROGRAMMER *) ldata(ln2);
            if(!is_programmer(pgm))
                continue;
            const char *pnam = (const char *) (pgm->id ? ldata(lfirst(pgm->id)) : "???");
    ...
    if (lsize(extended_params) > 0) {
        if (pgm->parseextparams == NULL) {
            for (LNODEID ln = lfirst(extended_params); ln; ln = lnext(ln)) {
                const char *extended_param = (const char *) ldata(ln);
     ...
     */
    for (ln= (LNODEID *) lfirst(updates); ln; ln= (LNODEID *) lnext(ln)) {
        upd = (UPDATE *) ldata(ln);
    ...
    if(!(p->prog_modes & PM_aWire)) { // not AVR32
        ...
        sig = NULL; // just force skip signature check
        if (sig == NULL)
